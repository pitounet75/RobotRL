 ...
 *
 * - **ISR**: drain RX FIFO0 → `xQueueSendFromISR`
 * - **Worker task**: match replies, invoke callbacks in task context
 * - **TX**: `odrive_can_hal_tx` under mutex
 *
 * **STM32H7**: call `odrive_can_fdcan_start()` before `odrive_can_async_init()`.
 * Hook `HAL_FDCAN_RxFifo0Callback` → `odrive_can_async_on_rx_fifo0_isr(hfdcan)`.
 *
 * **bxCAN (F4/F7)**: `HAL_CAN_Start()` + filters, then
 * `HAL_CAN_RxFifo0MsgPendingCallback` → `odrive_can_async_on_rx_fifo0_isr(hcan)`.
 */
