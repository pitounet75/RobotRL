...
{
#if defined(DEBUG_DESK_NO_ODRIVE)
    return;
#else
    g_app_odrive_rtos_boot_tried++;

    g_app_odrive_rtos_fdcan_ok = odrive_can_fdcan_ensure_started(&ODRIVE_CAN_HANDLE) ? 1u : 0u;
    if (g_app_odrive_rtos_fdcan_ok == 0u) {
        return;
    }

    if (g_odrive_startup_last_error == (uint32_t)ODRIVE_STARTUP_IN_PROGRESS) {
        (void)odrive_velocity_mode_startup(&ODRIVE_CAN_HANDLE);
    }
#endif
}
...
bool app_drivers_rtos_init(void)
{
    ...
    if (!odrive_can_async_init(&ODRIVE_CAN_HANDLE)) {
        return false;
    }
    ...
    if (!odrive_can_dma_init(&ODRIVE_CAN_HANDLE)) {
        return false;
    }

    if (!app_imu_init()) {
        app_halt_record(APP_HALT_IMU_INIT);
        /* Keep running so you can debug g_imu_init_err (-1 SPI, -2 bad WHO_AM_I). */
    }
    return true;
}
