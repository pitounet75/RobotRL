...
int main(void)
{
  ...
  /* Bench PC without CAN: add -DDEBUG_DESK_NO_ODRIVE to skip ODrive boot. */
  volatile uint8_t g_app_odrive_boot_skipped = 1u;
#else
  volatile uint8_t g_app_odrive_boot_skipped = 0u;
  if (!odrive_can_fdcan_start(&ODRIVE_CAN_HANDLE)) {
    app_halt_record(APP_HALT_FDCAN_START);
    Error_Handler();
  }

  /* RX IRQ stays off until odrive_velocity_mode_startup() finishes polling heartbeats. */
  if (!odrive_velocity_mode_startup(&ODRIVE_CAN_HANDLE)) {
